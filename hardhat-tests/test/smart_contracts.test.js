...
describe("Smart contracts test suite", function () {
    ...
    const ptauFilename = path.join("plonk_circuit", "powersOfTau15_final.ptau");
    ...
    it("Groth16 smart contract 1 input", async () => {
        expect(await groth16Verify(
            path.join("groth16", "circuit.r1cs"),
            path.join("groth16", "witness.wtns")
        )).to.be.equal(true);
    });

    it("Groth16 smart contract 3 inputs", async () => {
        expect(await groth16Verify(
            path.join("circuit2", "circuit.r1cs"),
            path.join("circuit2", "witness.wtns")
        )).to.be.equal(true);
    });

    it("Plonk smart contract 1 input", async () => {
        expect(await plonkVerify(
            path.join("plonk_circuit", "circuit.r1cs"),
            path.join("plonk_circuit", "witness.wtns")
        )).to.be.equal(true);
    });
    ...
    it("Fflonk smart contract 1 input", async () => {
        expect(await fflonkVerify(
            path.join("fflonk", "circuit.r1cs"),
            path.join("fflonk", "witness.wtns")
        )).to.be.equal(true);
    });
